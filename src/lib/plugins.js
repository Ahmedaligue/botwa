...
async function importModule(modulePath) {
	...
	try {
		...
		logger.error(`Error importing module ${modulePath}: ${error.message}`);
		throw error;
	}
}
...
export default class PluginsLoad {
	...
	async add(p, { silent = false } = {}) {
		try {
			...
			if (this.debug && !silent) {
				logger.success(`Loaded plugin: ${p}`);
			}
			...
			if (!silent) {
				logger.error(`Failed to load plugin ${p}: ${error.message}`);
			}
			return null;
		}
	}
	...
	async load() {
		...
		this.watcher.on("add", async (path) => {
			if (path.endsWith(".js")) {
				await this.add(path);
				logger.info(`Detected new plugin: ${path}`);
			}
		});
		...
		this.watcher.on("change", async (path) => {
			if (path.endsWith(".js")) {
				await this.add(path);
				logger.info(`Updated plugin: ${path}`);
			}
		});
		...
		this.watcher.on("unlink", (path) => {
			if (path in this.plugins) {
				...
				logger.warn(`Removed plugin: ${path}`);
			}
		});
